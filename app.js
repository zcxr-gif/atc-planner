...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW: Remove Default Leaflet Icon ---
...
async function initializeApp() {
    ...
    function createWaypointMarker(latlng, waypoint) {
        const iconHtml = '<span>▲</span>';
        const iconClass = 'waypoint-icon';
        const tooltipContent = `<b>${waypoint.name}</b><br>Type: ${waypoint.type}`;
    ...
    function restyleAllRunwaysAndLines() {
        ...
        if (activeAirportIcao) {
            displayAirportDetails(activeAirportIcao);
        } else {
            updateAirports();
        }
        Object.keys(planLayers).forEach(key => {
            ...
            if (layer.line) {
                let style = currentMapMode === "terrain"
                    ? FLIGHT_LINE_STYLES_TERRAIN[layer.lineType]
                    : FLIGHT_LINE_STYLES_REGULAR[layer.lineType];
                layer.line.setStyle(style);
            }
        });
    }
    ...
    function createMainPanel() {
        ...
        const content = `
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-top: 15px;">
                <a href="planner.html" class="button-link">Home</a>
