...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW HELPER FUNCTION TO GUARANTEE IMAGE LOADING ---
    /**
     * Loads an image and adds it to the map, returning a promise that resolves when complete.
     * @param {maptilersdk.Map} map The map instance.
     * @param {string} imageName The ID to give the image in the map style.
     * @param {string} imageUrl The URL path to the image file.
     * @returns {Promise<void>}
     */
    function loadImagePromise(map, imageName, imageUrl) {
   ...
   async function initializeWMM() {
    ...
    async function initializeApp() {
        ...
        // MODIFIED: Make the map load callback async to use await
        map.on('load', async () => {
            // --- MODIFICATION: Await the image loading to prevent race conditions ---
            try {
                await loadImagePromise(map, 'vor-compass-rose', 'vor_compass.png');
                console.log("SUCCESS: VOR compass rose image loaded and ready.");
            } catch (e) {
                console.error("CRITICAL: The 'vor_compass.png' image could not be loaded. VORs will not have icons.");
            }
            // --- END MODIFICATION ---
    ...
    function createTrafficScanPanel() {
        ...
        const content = `
            ...
            <div id="traffic-scan-results" class="info-card" style="display: none;"></div>
        `;
	...
	async function updateNavaids() {
        ...
        const navaidFeatures = navaids
            ...
            .map(navaid => ({
                ...
                properties: { name: navaid.name, type: navaid.type }
            }));
    ...
    async function updateAirportInfoPanel(airport, runways) {
        ...
        let runwaysHTML = `
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                <thead><tr style="text-align: left; border-bottom: 1px solid #555;"><th style="padding: 4px 2px;">Runway</th><th style="padding: 4px 2px;">Mag Hdg</th><th style="padding: 4px 2px;">True Hdg</th></tr></thead>
                <tbody>
        `;
        if (runways.length > 0) {
    ...
    function handleMouseMove(e) {
        ...
        source.setData({ type: 'LineString', coordinates: [[startPoint.lng, startPoint.lat], [currentPoint.lng, currentPoint.lat]] });
    ...
    function createFinalLine(start, end, stepId, altitude = '', speed = '', performCollisionCheck = false, lineType = 'standard', startAltitude, endAltitude, heading) {
        ...
        addSourceAndLayer(lineId, { type: 'geojson', data: { type: 'LineString', coordinates: [[start.lng, start.lat], [end.lng, end.lat]] } }, { type: 'line', paint: style });
        if (!heading) {
    ...
    function getAircraftIconPath(aircraftName, isSelected) {
        ...
        const aircraftMap = { 'a380': '/a380.png', '747': '/a380.png' };
        for (const key in aircraftMap) {
            if (lowerCaseName.includes(key)) return aircraftMap[key];
        }
        return '/plane.png';
    }
    ...
    function toggleDataBlockVisibility() {
        ...
        Object.values(planLabels).forEach(marker => { marker.getElement().style.visibility = visibility; });
    }
    ...
    function updateDataBlock(stepId) {
        ...
        const fullHtml = `<div class="flight-data-block" style="transform: translate(-50%, -50%) scale(${appSettings.dataBlockScale});"><div class="fdb-heading">${headingToShow}${headingUnit}</div><div class="fdb-row"><div class="fdb-data-item fdb-airspeed"><span class="fdb-value">${speed}</span><span class="fdb-unit">kts</span></div>${altitudeHtml}</div></div>`;
    ...
    async function getElevationAndMag(latlng) {
        let magVarText = "Mag Var: N/A";
        if (wmmModel) {
             magVarText = `Mag Var: ${wmmModel.field(latlng.lat, latlng.lng).declination.toFixed(2)}°`;
        }
        try {
            ...
            if (elevationMeters !== null && elevationMeters >= 0) {
                ...
                msaText = `MSA: ${Math.max(roundedAltitude, 2000).toLocaleString()}'`;
            }
    ...
    function getOptimalLabelPosition(start, end) {
        ...
        const distanceToAirport = turf.distance(turf.point([midPoint.lng, midPoint.lat]), turf.point([currentAirportCoords.lng, currentAirportCoords.lat]), { units: 'meters' });
    ...
    function createOrShowPlanPanel() {
        ...
        planPanel.querySelectorAll('.plan-section-header').forEach(headerToListenOn => {
            headerToListenOn.addEventListener('click', (e) => {
                const contentToShow = e.currentTarget.nextElementSibling;
                const isAlreadyVisible = contentToShow.style.display === 'block';
                document.querySelectorAll('.plan-section-content').forEach(s => s.style.display = 'none');
                if (!isAlreadyVisible) {
     ...
     function addPlanStep(stepId, heading, distanceMeters, altitude = '', speed = '', lineType = 'standard') {
        ...
        document.querySelectorAll('.plan-section-content').forEach(area => area.style.display = area.id === planContainer.id ? 'block' : 'none');
        ...
        headingSpan.addEventListener('click', () => {
            ...
            input.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') { e.preventDefault(); saveHeading(); }
                else if (e.key === 'Escape') { input.parentElement.replaceChild(headingSpan, input); }
            });
        });
                ...
                plugins: {
                    dragData: {
                        ...
                        onDragEnd: (e, datasetIndex, index, value) => {
                            if (index === 0) legData.startAltitude = value;
                            else legData.endAltitude = value;
                            legData.altitude = '';
                            updateAltitudeForLeg(stepId);
                            savePlanToLocalStorage();
                        }
